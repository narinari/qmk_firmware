...
void matrix_init(void) {
  ...
  i2c_init();
	...
	MCP23017_Mode_A(RIGHT_IO_ADDR, 0b00111111);
	MCP23017_Mode_B(RIGHT_IO_ADDR, 0b00000000);
...
static bool read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row)
{
    ...
    for(uint8_t col_index = 0; col_index < MATRIX_COLS-MATRIX_COLS_SPLIT; col_index++) {
        ...
        // Select the col pin to read (active High)
...
static void select_row(uint8_t row)
{
    ...
    MCP23017_Write_B(RIGHT_IO_ADDR, (1<<row));
}
...
static void unselect_row(uint8_t row)
{
    ...
    MCP23017_Write_B(RIGHT_IO_ADDR, 0);
}
